# To view this Streamlit app on a browser, run it with the following command in terminal:
# streamlit run *your file path*/app.py
...
def load_models():
    ...
    return en_model, en_tokenizer

en_model, en_tokenizer = load_models()
...
# Tiêu đề đề tài
st.title('Phân tích trạng thái bằng mô hình BERT (English only)')
...
with tab1:
    ...
    if st.button('Phân tích trạng thái', key='single'):
        if text_input:
            ...
            save_history(pd.DataFrame([{'Text': text_input, 'Sentiment': sentiment, 'Timestamp': timestamp}]))
        else:
    ...
    if st.button('Phân tích trạng thái', key='multiple'):
        ...
        for i, text in enumerate(texts):
            if text:
                ...
                timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                results.append({"Text": text, "Sentiment": sentiment, "Timestamp": timestamp})
            else:
    ...
    if uploaded_file is not None:
        ...
        if st.button('Phân tích trạng thái', key='csv'):
            with st.spinner('Đang phân tích ...'):
                ...
                save_history(df[[column_to_analyze, 'sentiment', 'timestamp']].rename(columns={column_to_analyze: 'Text', 'sentiment': 'Sentiment', 'timestamp': 'Timestamp'}))
...
st.sidebar.info("""Chương trình này sử dụng mô hình BERT để phân tích trạng thái của văn bản được đưa vào (English only).
                   Có thể xử lý văn bản đơn lẻ, nhiều văn bản được nhập thủ công hay nhiều văn bản từ 1 file CSV.""")
